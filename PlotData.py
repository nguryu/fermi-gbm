...
for i in range(len(f_galaxy)):
    # Galaxy_XAE.dat
    if log10(f_galaxy[i]) >= -4:
        log_f_galaxy.append(log10(f_galaxy[i]))
        log_a_real.append(log10(abs(a_real[i])**2))
    ...
    if log10(res_f_galaxy[i]) >= -4:
        ...
        log_res_a_real.append(log10(abs(res_a_real[i])**2))
        res_y.append(log10(res_a_real[i]**2 + res_a_im[i]**2))  # Calculated residual y_ax axis
        res_a_real_cut.append(a_real[i])
        res_a_im_cut.append(a_im[i])
...
def white_amp_vs_freq(log_f_galaxy, log_res_f_galaxy, white_real, white_im, white_res_real, white_res_im):
    ...
    sub1.title.set_text('A vs. f')
    sub1.plot(log_f_galaxy, log_white_y, color='gray', alpha=1)

    # Subplot 2
    sub2 = plt.subplot(312)
    #sub2.set_xlim([-4, -1.2])
    #sub2.set_ylim([1.5, 2.5])
    sub2.title.set_text('Residual A vs. Residual time')
    ...
    sub2.plot(log_res_f_galaxy, log_white_res_y, color='red', alpha=0.1)
    ...
    sub3.title.set_text('Superimposed')
    sub3.set_xlabel(r'$\rm{log}(f)$', visible=True)
    sub3.plot(log_f_galaxy, log_white_y, color='gray', alpha=1)
    sub3.plot(log_res_f_galaxy, log_white_res_y, color='red', alpha=0.1)

    # Output
    fig2.savefig('white_amp_vs_freq_log.eps', format='eps', bbox_inches='tight', pad_inches=0.02, dpi=1000)
...
def ifft_white(log_f_galaxy, log_res_f_galaxy, white_real, white_im, white_res_real, white_res_im):
    ...
    f_galaxy = []
    res_f_galaxy = []
    # Return to regular frequency instead of the logarithmic form.
    for i in range(len(log_f_galaxy)):
    ...
    ifft_real = fft.ifft(white_real)
    ifft_im = fft.ifft(white_im)
    ifft_res_real = fft.ifft(white_res_real)
    ifft_res_im = fft.ifft(white_res_im)
    ifft_y = []
    ifft_res_y = []
    log_ifft_y = []
    log_ifft_res_y = []
    ...
    for i in range(len(log_f_galaxy)):
        ifft_y.append(ifft_real[i] + ifft_im[i])
        ifft_res_y.append(ifft_res_real[i] + ifft_res_im[i])
        log_ifft_y.append(log10(ifft_real[i] + ifft_im[i]))
        log_ifft_res_y.append(log10(ifft_res_real[i] + ifft_res_im[i]))
    ...
    sub1.plot(x_ax, log_ifft_y, color='gray', alpha=1)
    ...
    sub2.set_ylabel(r'$w(t) = \rm{log}(\mathcal{L}^{-1}(w(f)))$', visible=True)
    sub2.plot(res_x, log_ifft_res_y, color='red', alpha=0.1)
    ...
    sub3.plot(x_ax, log_ifft_y, color='gray', alpha=1)
    sub3.plot(res_x, log_ifft_res_y, color='red', alpha=0.1)

    # Output
    fig3.savefig('ifft_white_log.eps', format='eps', bbox_inches='tight', pad_inches=0.02, dpi=1000)
